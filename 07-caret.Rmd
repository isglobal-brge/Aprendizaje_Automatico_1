...
nzv
filteredDatos <- select(datos, -all_of(nzv))
dim(datos)
dim(filteredDatos)
```
...
**Paso 6:** Ahora centraremos y normalizaremos los datos (esto último no tiene porqué sere necesario). Recordamos que también se pueden transformar los datos para garantizar normalidad usando `preProcess()` con el métodos Box-Cox o Yeo-Johnson de `caret()`. 
...
**Paso 7**: La binarización de variables no es necesario

**Paso 8**: Aprendizaje de las transformaciones de pre-procesado y aplicación a nuestros conjuntos de datos. Empezamos con el entrenamiento
